  ...
  try {
    ...
    const [linkedDeals, linkedQuotes, linkedPayments, linkedCredits] = await Promise.all([
      ...
      db.select({ id: paymentsTable.id }).from(paymentsTable)
        .where(and(eq(paymentsTable.customer_id, String(id)), isNull(paymentsTable.deleted_at)))
        .limit(1),
      db.select({ id: creditsTable.id }).from(creditsTable)
        .where(and(eq(creditsTable.customer_id, String(id)), isNull(creditsTable.deleted_at)))
        .limit(1),
    ]);
