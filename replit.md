...
docs/quotes-module.md — full detailed spec for the quotes module (wizard, snapshots, versioning, duplication). Read it before touching anything quote-related.
...
Do not build a generic project-management tool. Do not add features outside the approved milestone.
...
Allowed English in UI only for real technical values: IDs, emails, URLs, API keys, environment variable names and developer/debug values.
...
See docs/quotes-module.md for the full implementation of this rule (party_snapshot / items_snapshot / totals_snapshot / terms_snapshot / notes_snapshot).
...
How to test it
Known limitations
Whether any visible English text remains in the UI
...
Existing tables (19): app_users, audit_logs, component_resources, components, credits, customers, deals, leads, monday_import_records, payments, product_components, products, quote_components, quote_products, quote_status_history, quote_versions, quotes, resources, special_tasks. Views: v_deal_summary, v_quote_summary.

Key structure:

Quotes are versioned via the quote_versions table (party_snapshot / items_snapshot / totals_snapshot / terms_snapshot / notes_snapshot JSONB columns) plus quotes.current_version_id — this is the sole versioning mechanism to use going forward, and implements the Quote History Rule at DB level. Full column-level detail is in docs/quotes-module.md.
quotes also has legacy quote_version_group_id / version_number / created_from_quote_id columns from an earlier versioning approach (each version was a separate quotes row). Do not use these columns in new logic — do not remove them either.
quote_products / quote_components hold the live, editable draft representation of a quote's line items (original_*/*_snapshot columns, customer_note / internal_note, price_override_reason, quantity_change_reason). They are used for draft editing only — the source of truth for a sent/approved quote version is always quote_versions.items_snapshot.
quotes.discount_amount is the only discount field that exists — a single discount at the whole-quote level. There is no per-line discount column anywhere.
quotes also has three separate note columns: customer_notes (shown to client), operation_notes (internal, operations team), internal_notes (general internal).
...
See docs/quotes-module.md before making any change to the quotes module.
